...
'SumÕs should be as double because we donÕt want to lose any decimal places in calculations
Sub OpenPriceSums()
    Dim ws As Worksheet
    Dim ticker As Range
...
'For each unique marker the corresponding sum starts at 0
        For Each ticker In ws.Range("J2", ws.Cells(ws.Rows.Count, "J").End(xlUp))
            ' need the sum to start out at 0, it wonÕt do it automatically unless you say it

            totalSum = 0

'For each opening price in the range of column C, if it is not empty and if the value of that rowÕs A column equals the string letter value from our ticker column, the cells for J, then add the value to our opening sum for that ticker. Aka, add it to the column next to the proper ticker in Cell J.
...
'Need to do this for every sheet in the workbook, for each ticker in column J, we will use the open price drawn from the open price totals for that ticker Ôrow from column K
...
'It could be ticker.Row, M instead of ticker.Row, 1, but apparently ticker.Row, 1 means same row as the ticker, and Òfirst columnÓ of the ÔYearlyChangeColumn, there is only one Yearly Change Column so itÕs kinda silly, but it still works and would provide more flexibility if we needed multiple columns of deciphering in the yearly change chart
...
'Okie dokie so when I tried this for the whole workbook instead of just sheet A it froze my excel, and I donÕt really need to run it for the whole
...
'if the stock value, which then becomes the volumeValue, is bigger than the maxVolume(which starts as Ôzero), then the maxVolume becomes the volumeValue and continues from there. The next value will have Ôto be bigger in order to override it.
            If volumeValue > maxVolume Then
                maxVolume = volumeValue

'Get the ticker associated with it, the value in the tickers column, which we have defined as column J, the Ôfirst and only column of that set, row=same row as stockvol. ItÕs within the loop though so it will keep Ôchanging according to the previous.
